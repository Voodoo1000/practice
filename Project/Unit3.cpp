...
void __fastcall TForm3::Edit3Exit(TObject *Sender)
{
	...
	if (containsLetters || containsSymbols || !containsDigits) {
		edit->SetFocus();
		ShowMessage("Не допускается ввод букв и символов, кроме тире в начале числа, и поле должно содержать хотя бы одну цифру");
		edit->Text = "";
		return;
	}
...
void __fastcall TForm3::Edit5Exit(TObject *Sender)
{
	...
	if (number > 500 || number < 4) {
		edit->SetFocus();
		ShowMessage("Число не может превышать 500 и не может быть меньше 4");
		edit->Text = "";
		return;
	}
}
